...
class SplineWeightLayer(nn.Module): # neurone cell body
    def __init__(self, in_features=187, in_hidden=64, out_hidden=64, kernel_size=15,
        ...
        if use_self_attention:
            self.WQ = nn.Parameter(torch.Tensor(out_hidden, out_hidden))
            self.WK = nn.Parameter(torch.Tensor(out_hidden, out_hidden))
        else:
    ...
    def forward(self, x, attention_temperature=None):
        b, seq_len, h_dim = x.shape
        x_norm = self.ln(x)
        x_prime = torch.matmul(x_norm, self.W2) + self.b
        W1 = None
        if self.use_self_attention:
            Q = torch.matmul(x_prime, self.WQ)
            K = torch.matmul(x_prime, self.WK)
            Q = F.layer_norm(Q, [Q.size(-1)])
            K = F.layer_norm(K, [K.size(-1)])
            raw_attn = torch.bmm(Q, K.transpose(-1, -2))
            W1 = F.softmax(
                raw_attn / (np.sqrt(Q.shape[-1]) * attention_temperature), dim=-1)
        self.last_W1 = W1.detach() if W1 is not None else None
        ...
        return spatial_combined + self.gamma * res_x
...
class FastKANLayer(nn.Module):   # edge
    def __init__(self, in_neurons, out_neurons, grid_size=64, spline_order=3, hidden_dim=64, neuron_out_dim=64,
                 kernel_size=15, seq_len=187, use_self_attention=True):
        ...
        self.weight_layers = nn.ModuleList([
            SplineWeightLayer(in_features=seq_len, in_hidden=hidden_dim, out_hidden=neuron_out_dim,
                              kernel_size=kernel_size, use_self_attention=use_self_attention)
    ...
    def forward(self, x_in):
        ...
        for j in range(self.out_neurons):
            ...
            out_j = self.weight_layers[j](
                combined_input,
                attention_temperature=t_val if self.use_self_attention else None)
            if hasattr(self, 'visualize_idx') and j == self.visualize_idx:
                self.last_W1 = self.weight_layers[j].last_W1
...
class ECGKANModel(nn.Module):   # main E-KAT model
    def __init__(self, grid_size=64, spline_order=3, seq_len=187):
        ...
        self.layer1 = FastKANLayer(1, 4, grid_size, spline_order, hidden_dim=64, neuron_out_dim=64, kernel_size=25, seq_len=seq_len, use_self_attention=False)
        self.layer2 = FastKANLayer(4, 8, grid_size, spline_order, hidden_dim=64, neuron_out_dim=128, kernel_size=15, seq_len=seq_len, use_self_attention=True)
        self.layer3 = FastKANLayer(8, 8, grid_size, spline_order, hidden_dim=128, neuron_out_dim=32, kernel_size=9, seq_len=seq_len, use_self_attention=True)
        self.layer4 = FastKANLayer(8, 5, grid_size, spline_order, hidden_dim=32, neuron_out_dim=1, kernel_size=3, seq_len=seq_len, use_self_attention=True)
