...
class ImagePrepConv(nn.Module):
    def __init__(self, in_channels=4, out_channels=32):
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 4, kernel_size=11, padding=5),
            nn.BatchNorm2d(out_channels // 4),
            nn.SiLU())
        self.branch4 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 4, kernel_size=15, padding=7),
            nn.BatchNorm2d(out_channels // 4),
            nn.SiLU())
        ...
        self.relu = nn.SiLU()
        self.pixel_unshuffle = nn.PixelUnshuffle(downscale_factor=6)
    ...
    def forward(self, x):   # x: [B, 5, 32, 32]
        ...
        x3 = self.branch3(x)
        x4 = self.branch4(x)
        out = torch.cat([x1, x2, x3, x4], dim=1)  # [B, 64, 32, 32]
...
class BUSKANModel(nn.Module):   # main E-KAT model
    def __init__(self, grid_size=64, spline_order=3):
        super(BUSKANModel, self).__init__()
        self.prep_conv = ImagePrepConv(in_channels=4, out_channels=32)
        
        self.layer1 = FastKANLayer(1, 4, grid_size, spline_order, hidden_dim=1152, neuron_out_dim=576, kernel_size=7)
        self.layer2 = FastKANLayer(4, 8, grid_size, spline_order, hidden_dim=576, neuron_out_dim=288, kernel_size=5)
        self.layer3 = FastKANLayer(8, 16, grid_size, spline_order, hidden_dim=288, neuron_out_dim=144, kernel_size=5)
        self.layer4 = FastKANLayer(16, 16, grid_size, spline_order, hidden_dim=144, neuron_out_dim=128, kernel_size=3)
        self.layer5 = FastKANLayer(16, 20, grid_size, spline_order, hidden_dim=128, neuron_out_dim=128, kernel_size=3)
        self.layer6 = FastKANLayer(20, 8, grid_size, spline_order, hidden_dim=128, neuron_out_dim=16, kernel_size=3)
        self.layer7 = FastKANLayer(8, 2, grid_size, spline_order, hidden_dim=16, neuron_out_dim=1, kernel_size=1)
...
def visualize_attention_images(
    raw_img,
    col_ratio,
    label_name,
    layer_idx=None,
    ...
    """
    Display one BUS-BRA grayscale image and its W1 attention overlay.

    Parameters
    ----------
    raw_img:
        2-D grayscale image [H, W], or tensor/array containing channels.
        If [C, H, W] is supplied, channel 0 is used.
    col_ratio:
        Length-256 token attention vector returned by
        visualize_w1_analysis().
    label_name:
        Display label, for example "benign" or "malignant".
    layer_idx:
        Selected E-KAT layer number.
    neuron_idx:
        Selected neuron number within the layer.
    save_path:
        Optional output PNG path.
    """
    raw_img = np.asarray(raw_img, dtype=np.float32)
    # Accept [H,W], [1,H,W], or [C,H,W].
    if raw_img.ndim == 3:
        raw_img = raw_img[0]
    if raw_img.ndim != 2:
        raise ValueError(
            f"raw_img must have shape [H,W] or [C,H,W], got {raw_img.shape}"
        )
    # Display normalization only; this does not change model inference.
    finite_mask = np.isfinite(raw_img)
    if not finite_mask.any():
        raise ValueError("raw_img contains no finite values")
    valid_values = raw_img[finite_mask]
    image_min = float(valid_values.min())
    image_max = float(valid_values.max())
    if image_max > image_min:
        image_show = (raw_img - image_min) / (image_max - image_min)
    else:
        image_show = np.zeros_like(raw_img, dtype=np.float32)
    col_ratio = np.asarray(col_ratio, dtype=np.float32).reshape(-1)
    if col_ratio.size != 256:
        raise ValueError(
            f"Expected 256 W1 token weights for a 16x16 token grid, "
            f"got {col_ratio.size}"
        )
    # Convert 256 token weights back to the 16×16 spatial token grid.
    attention_16 = col_ratio.reshape(16, 16)
    attention_min = float(attention_16.min())
    attention_max = float(attention_16.max())
    attention_16 = (
        (attention_16 - attention_min)
        / (attention_max - attention_min + 1.0e-8)
    )
    # Upsample 16×16 token attention to the BUS-BRA image resolution.
    attention_map = torch.from_numpy(attention_16).view(1, 1, 16, 16)
    attention_map = F.interpolate(
        attention_map,
        size=image_show.shape,
        mode="bilinear",
        align_corners=False,
    )[0, 0].numpy()
    # Suppress weak background responses visually.
    display_threshold = np.percentile(attention_map, 20.0)
    attention_alpha = np.clip(
        (attention_map - display_threshold)
        / (attention_map.max() - display_threshold + 1.0e-8),
        0.0,
        1.0,
    )
    attention_alpha = 0.15 + 0.70 * attention_alpha
    figure, axes = plt.subplots(
        1,
        2,
        figsize=(15, 7),
        gridspec_kw={"width_ratios": [1.0, 1.07]},
    )
    axes[0].imshow(image_show, cmap="gray", vmin=0.0, vmax=1.0)
    axes[0].set_title(f"Input breast ultrasound: {label_name}", fontsize=12)
    axes[0].axis("off")
    axes[1].imshow(image_show, cmap="gray", vmin=0.0, vmax=1.0)
    ...
    axes[1].set_title("W1 Attention Heatmap (Overlaid)", fontsize=12)
    axes[1].axis("off")
    ...
    plt.colorbar(
        heatmap_artist,
        cax=color_axis,
        label="Normalized attention intensity",
    )
    layer_text = ""
    if layer_idx is not None and neuron_idx is not None:
        layer_text = f" | Layer {layer_idx}, neuron {neuron_idx}"
    figure.suptitle(
        f"Audit Analysis: {label_name}{layer_text}",
        fontsize=16,
        y=0.96,
    )
    figure.tight_layout()
    if save_path is not None:
        ...
        print(f"Saved attention overlay: {destination}")
    plt.show()
    return figure, attention_map
...
def analyze_image_representation(model, dataloader, device, n_samples=3000):
    ...
    all_labels = []
    collected = 0
    cifar10_classes = ["benign","malignant"]

    with torch.no_grad():
        for batch_data, batch_label in dataloader:
            input_4ch = batch_data.to(device)
            latent = model(input_4ch, return_latent=True,)
...
def calculate_model_complexity(model, device):
    model.eval()
    dummy_input = torch.randn(1, 4, 96, 96).to(device)
    try:
...
if __name__ == "__main__":
    ...
    model = BUSKANModel(grid_size=64, spline_order=3).to(device)
    train_loader, test_loader, dataset_info = get_busbra_dataloaders(
        busbra_root_dir="/content/drive/MyDrive/BUS/BUSBRA",
