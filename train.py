...
def kan_optimal_loss(outputs, targets, model, epoch,
                     lamb=5e-4,  # 样条权重正则系数 (L1+L2)
                     mu=5e-4,  # 路径熵系数 (引导特征解耦)
                     gamma_smth=8e-2,  # 样条平滑度系数 (抑制高频噪声)
                     mu_att=1e-3,  # 注意力矩阵 W1 稀疏系数
                     reg=1e-5,  # SplineWeightLayer 标量稳定性系数
                     label_smooth=0):  # 逻辑标签平滑系数 (遏制 BCE 反弹)
    """
    针对脑电信号优化的 KAN 损失函数。
    集成了：手动标签平滑、RMS 路径强度对齐、二阶导数平滑。
    """
    device = outputs.device
    # 从模型实例中获取预计算的 pos_weights
    pos_weights = getattr(model, 'pos_weights', None)

    # --- 1. 手动实现逻辑标签平滑 (Label Smoothing) ---
    # 目的：限制输出 Logits 增长，防止模型过度自信导致的验证集 BCE 反弹
    # 虽然预处理有过时域平滑，这里的逻辑平滑是针对 BCE 数值稳定性的最后一道防线
    with torch.no_grad():
        # 将 [0, 1] 目标缩放至 [0.05, 0.95] (假设 label_smooth=0.1)
        smoothed_targets = targets * (1.0 - label_smooth) + 0.5 * label_smooth

    # 使用手动平滑后的标签进行计算，不再直接传参给函数以确保兼容性
...
def train_gal_model(model, train_loader, val_loader, epochs=100, lr=1e-3, resume_path=None):
    ...
    ], weight_decay=1e-5)  # 修正：weight_decay 改为 1e-5
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            print(f">>> 成功从断点恢复，起始 Epoch: {start_epoch}")
        else:
            # 如果文件里只有权重本身
            model.load_state_dict(checkpoint)
            start_epoch = 4
            best_val_acc = 95.36
            print(">>> 警告：未发现优化器状态，仅恢复模型权重。")
    ...
    for epoch in range(start_epoch, epochs):
        model.train()
        # 1. 初始化累加器：区分 BCE (业务损失) 和 Total (含正则项的总损失)
        t_bce_sum = 0
        t_total_sum = 0
        t_correct_sum = 0
        processed_batches = 0
...
if __name__ == "__main__":
    ...
    PATH_past = "/content/drive/MyDrive/GAL_KAN_Project/best_gal_model_04.pth"
    # 包含所有 12 个受试者
    all_subjects = list(range(1, 13))

    # 定义划分逻辑：1-3, 5-7 训练；4, 8 测试
    train_series = [1, 2, 3, 5, 6, 7]
    val_series = [4, 8]

    # 2. 计算全局类别平衡权重 (pos_weights)
    # 跨受试者计算以获取更稳健的比例
    print(f">>> 正在针对所有受试者计算全局类别平衡权重...")
    p_weights = calculate_pos_weights(PATH_train, all_subjects, range(1, 9)).to(device)
    print(f">>> 全局 pos_weights: {p_weights}")

    # 3. 加载训练集（合并所有人 1-3, 5-7）
    # WAYEEGDataset 内部会自动遍历传入的 subject_ids 列表并拼接数据
    print(f"\n>>> 正在加载全员训练集 (Series: {train_series})...")
    ...
    # 4. 构建全局统计字典
    # 必须使用训练集的均值和标准差来标准化验证集，防止数据泄露
    current_stats = {
        'mean': train_ds.mean,
        'std': train_ds.std
    }
    print(f">>> 全局 Mean Dtype: {train_ds.mean.dtype} | 全局 Std Dtype: {train_ds.std.dtype}")

    # 5. 加载验证集（合并所有人 4, 8）
    print(f"\n>>> 正在加载全员验证集 (Series: {val_series})...")
    ...
    # 6. 构建 DataLoader
    # 增加 num_workers 建议：由于数据量翻倍，建议设置 num_workers > 0 提升加载速度
    train_loader = DataLoader(train_ds, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=32, shuffle=False, num_workers=4, pin_memory=True)

    # 7. 模型初始化与训练
    # 建议给模型注入 seq_len 确保线性层维度匹配
    model = BCIKANModel().to(device)
    model.pos_weights = p_weights

    print(f"\n>>> 数据准备就绪。训练样本数: {len(train_ds)} | 验证样本数: {len(val_ds)}")
